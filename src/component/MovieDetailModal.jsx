...
export default function MovieDetailModal({movie, setDetailView}) {
    ...
    return <div className="fixed inset-0 flex justify-center items-center w-screen h-full  backdrop-blur-xl">
        <div className=" bg-gray-600 w-md h-[90%] flex-col overflow-scroll scrollbar-none rounded-xl shadow-lg">
            <button onClick={()=>setDetailView(false)} className="bg-white text-black sticky top-0 p-1.5 rounded-2xl left-full "><X /></button>
            ...
            <div className="mx-4 my-0.5 flex-col relative font-stretch-50%">
                ...
                <div className="my-2 relative">{movie.summary}</div>
                <button onClick={()=>setDetailView(false)} className=" relative w-full mx-auto my-2 text-center  bg-white text-black font py-0.5 px-4 rounded-xl hover:bg-amber-400 hover:font-semibold whitespace-nowrap">
                    Close
                </button>
            </div>
        </div>
